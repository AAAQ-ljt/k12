<template>
  <BaseDialog
    ...
    @close="handleClose"
  >
    <div v-loading="loading" class="homework-review-dialog">
      <template v-if="detail">
        <section class="homework-review-dialog__summary">
          <div class="summary-card">
            <div class="summary-card__title">
              <strong>{{ detail.paperName || detail.lessonName }}</strong>
            ...
            >
              <div
                ...
                :class="[
                  'question-option',
                  {
                    'question-option--selected': isOptionSelected(
                      option,
      ...
      <el-empty v-else description="暂无作业详情" />
    </div>
  </BaseDialog>
</template>
...
import {
  getCourseHomeworkSubmitDetail,
  judgeCourseHomeworkSubmit,
} from '@/api/course'
...
const dialogTitle = computed(() =>
  isJudgeMode.value ? '批改学生作业' : '查看学生作业'
)
...
const loadDetail = async () => {
  if (!props.courseId || !props.lessonId || !props.studentId) {
    resetState()
    return
  }
  loading.value = true
  const result = await getCourseHomeworkSubmitDetail({
...
const handleSave = async () => {
  if (!detail.value?.submitId) {
    ElMessage.warning('当前作业尚未提交，无法批改')
    return
  }
  const subjectiveQuestionList = (detail.value.sectionList || []).flatMap(
    (section) =>
      (section.questionList || []).filter((question) =>
  ...
  const result = await judgeCourseHomeworkSubmit(payload)
  if (!result) {
    return
  }
...
.homework-review-dialog__summary {
  display: flex;
}
