...
- 响应拦截器：
  - 成功响应直接返回业务数据；
  - 管理端：登录失效响应码统一清登录态并跳转登录页（携带 `redirect`），业务页面不要重复处理；
  - 学生端：收到 401 统一清登录态并弹出全局登录弹窗（不强制跳转），登录成功后回跳原目标页；
...
- 守卫流程（管理端）：未登录跳登录页 → 已登录但无用户信息时拉取 → 校验菜单编码 → 通过则放行（用守卫组件 / loader 实现，不在页面里写跳转逻辑）。
- 守卫流程（学生端，Codex 模式）：公开页（AI 助手 / 编程环境）免登录可浏览；受保护页（学习路径 / 课程教材 / 我的）未登录 → 弹出全局登录弹窗，不强制跳转；登录成功后自动回跳原本想访问的页面。
...
- **登录入口（已确认，Codex 模式）**：左下角固定入口——未登录显示「登录」按钮，点击弹出登录弹窗；已登录显示头像/昵称，点击展开下拉菜单（个人中心、退出登录）。
- **登录 / 注册弹窗**：统一组件，登录表单（账号 / 密码）⇄ 注册表单（用户名 / 邮箱 / 密码 / 确认密码）可切换；注册成功自动登录；注册默认学段 `PRIMARY_LOW`（小学低年级），可在「我的」页面修改学段。
