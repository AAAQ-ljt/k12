...
- 交互映射：小低 → 语音 / 绘本 / 游戏；小高 → 图文 + 绘本 + 闯关；初中 → 项目式 + 编程入门；高中 → 算法讲解 + 编程实践 + 项目式学习。
...
- AI 模型分工（已确认）：对话 / 意图路由 / SVG 动画 / 编程辅助 / 学习路径生成 = DeepSeek API（`NEXORA_DEEPSEEK_API_KEY`）；知识向量化 = 阿里百炼 qwen3.7-text-embedding（`NEXORA_DASHSCOPE_API_KEY`）；绘本插图 = 阿里百炼 qwen-image-3.0（`NEXORA_DASHSCOPE_API_KEY`）；绘本旁白语音合成 = 小米 MiMo mimo-v2.5-tts（`NEXORA_MIMO_TTS_API_KEY`，音色白名单校验，接口说明见 `docs/mimo-v2.5-tts接口文档.md`）。
