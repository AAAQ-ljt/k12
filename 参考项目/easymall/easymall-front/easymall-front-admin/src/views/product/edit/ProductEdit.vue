<template>
  <div class="form-style" label-width="auto" @submit.prevent>
...
const submitProduct = async () => {
  ...
  for (let [index, sku] of productEditStore.skuList.entries()) {
    ...
    skuListResultData.push(resultData)
  }
  const submitData = {
  ...
  let result = await proxy.Request({
    url: route.params.productId
      ? proxy.Api.updateProduct
      : proxy.Api.addProduct,
    dataType: 'json',
    params: submitData,
  })
  if (!result) {
