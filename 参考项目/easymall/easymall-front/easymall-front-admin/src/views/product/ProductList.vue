      ...
      <Table ref="tableInfoRef" :columns="columns" :fetch="loadDataList" :dataSource="tableData">
        ...
        <template #slotOp="{ index, row }">
          <div class="list-op-panel" v-if="row.status != -1">
            ...
            <OpBtn icon="icon-commend" tips="推荐" @click="commend(row)" v-if="row.commendType == 0"></OpBtn>
            <OpBtn icon="icon-cancel-commend" tips="取消推荐" @click="commend(row)" v-if="row.commendType == 1"></OpBtn>
            <OpBtn icon="icon-stock" tips="更新库存" @click="updateStock(row)"></OpBtn>
            <OpBtn :icon="row.status == 0 ? 'icon-shangjia' : 'icon-xiajia'" :tips="row.status == 0 ? '上架' : '下架'"
              @click="changeStatus(row)">
            </OpBtn>
            <OpBtn icon="icon-edit" tips="修改" @click="proxy.Utils.jump(`/product/updateProduct/${row.productId}`)">
            </OpBtn>
            <OpBtn icon="icon-delete" type="danger" tips="删除" @click="del(row)"></OpBtn>
          </div>
        </template>
      </Table>
    </div>
